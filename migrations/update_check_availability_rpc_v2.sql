...
BEGIN
  ...
  zone_occupancy AS (
     ...
     AND EXISTS (
        ...
        -- We don't strictly need to check pax for occupancy, as any booking on a valid table consumes it
        -- But keeping consistency with previous logic if needed. 
        -- Actually, if a table is occupied, it's occupied regardless of pax.
        -- However, the previous logic filtered by valid tables for THIS request.
        -- So we only care if a table THAT COULD HOST THIS REQUEST is occupied.
