-- Drop previous version (v7)
DROP FUNCTION IF EXISTS check_availability(date, turn_type, integer, uuid, boolean);

-- Recreate with explicit aliases to avoid ambiguity
...
BEGIN
  ...
  valid_tables AS (
     SELECT t.id AS vt_table_id, t.zone_id AS vt_zone_id, t.min_pax, t.max_pax
     FROM tables t
  ...
  zone_capacity AS (
     SELECT vt.vt_zone_id AS zc_zone_id, COUNT(*) AS total_tables
     FROM valid_tables vt
     GROUP BY vt.vt_zone_id
  ),
  -- 3. Count occupied tables
  zone_occupancy AS (
     SELECT b.zone_id AS zo_zone_id, COUNT(*) AS booked_tables
     FROM bookings b
  ...
  blocked_zones AS (
     SELECT DISTINCT b.zone_id AS bz_zone_id
     FROM bookings b
     WHERE b.restaurant_id = check_restaurant_id
     AND b.booking_date = check_date
     AND b.turn = check_turn
  ...
  FROM zones z
  LEFT JOIN zone_capacity zc ON zc.zc_zone_id = z.id
  LEFT JOIN zone_occupancy zo ON zo.zo_zone_id = z.id
  WHERE z.restaurant_id = check_restaurant_id
  -- Exclude blocked zones (using aliased column in subquery)
  AND z.id NOT IN (SELECT bz_zone_id FROM blocked_zones)
