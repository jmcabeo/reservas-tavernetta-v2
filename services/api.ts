...
export const createBooking = async (formData: BookingFormData, isWaitlist: boolean = false): Promise<{ success: boolean; error?: string; bookingId?: string }> => {
  ...
  // If no table is assigned (public booking) and it consumes capacity, find a free table.
  ...
  if (!finalAssignedTableId && (formData.consumes_capacity !== false) && formData.zone_id) {
    try {
      // 1. Get all suitable tables in this zone for this pax
      const { data: suitableTables } = await supabase
        .from('tables')
        .select('id')
      ...
      if (suitableTables && suitableTables.length > 0) {
        // 2. Get occupied table IDs for this date/turn
        ...
        if (freeTable) {
          finalAssignedTableId = freeTable.id;
          console.log(`[Auto-Assign] Assigned Table ID ${finalAssignedTableId} for booking.`);
        } else {
          console.warn('[Auto-Assign] No free table found, creating booking without specific table assignment.');
        }
      }
    } catch (err) {
      console.error('[Auto-Assign] Error finding free table:', err);
    }
  }
    ...
    .insert([
      {
        ...
        assigned_table_id: finalAssignedTableId, // Save assigned table (manual or auto)
        consumes_capacity: formData.consumes_capacity  // Save capacity flag
      }
    ]);
